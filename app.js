async function waitForTelegramWebApp(timeoutMs = 2200) {
  const started = Date.now();
  while (Date.now() - started < timeoutMs) {
    const tg = window.Telegram?.WebApp || window.__BNEO_TG_WEBAPP__ || null;
    if (tg) return tg;
    await new Promise((resolve) => setTimeout(resolve, 120));
  }
  return window.Telegram?.WebApp || window.__BNEO_TG_WEBAPP__ || null;
}

document.addEventListener('DOMContentLoaded', async () => {
  const tg = await waitForTelegramWebApp();
  const bridgeMeta = window.__BNEO_TG_CONTEXT__ || {};
  ...
  function sendToBot(raw = null) {
    ...
    if (!isTelegram || typeof tg.sendData !== 'function') {
      setStatus('Встроенный Telegram bridge не найден. Черновик можно скопировать вручную.', 'error');
      return;
    }
    try {
      tg.sendData(payload);
  ...
  if (isTelegram) {
    ...
    if (tg.MainButton) {
      ...
      try { tg.MainButton.onClick(() => sendToBot()); } catch (_) {}
    }
    if (tg.SecondaryButton) {
      try { tg.SecondaryButton.offClick(copyDraft); } catch (_) {}
      try { tg.SecondaryButton.onClick(copyDraft); } catch (_) {}
    }
    if (htmlActionsEl) htmlActionsEl.style.display = 'none';
    const source = bridgeMeta.source ? ` Источник: ${bridgeMeta.source}.` : '';
    setStatus(`Приложение BNEO подключено.${source} Заполните форму и нажмите «Применить».`, 'success');
  } else {
    const source = bridgeMeta.source ? ` Источник: ${bridgeMeta.source}.` : '';
    setStatus(`Открыт режим браузера: объект Telegram WebApp не найден.${source} Сборка черновика и копирование доступны.`, 'info');
  }
});
