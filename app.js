document.addEventListener('DOMContentLoaded', () => {
  ...
  const hasTelegramNamespace = Boolean(window.Telegram);
  ...
  if (isTelegram) {
    ...
    const platform = tg.platform ? ` Платформа: ${tg.platform}.` : '';
    setStatus(`Приложение BNEO подключено.${platform} Заполните форму и нажмите «Применить».`, 'success');
  } else {
    const reason = hasTelegramNamespace
      ? 'Telegram namespace есть, но объект WebApp не передан клиентом.'
      : 'объект Telegram WebApp не найден.';
    setStatus(`Открыт режим браузера: ${reason} Сборка черновика и копирование доступны. Для отправки запускайте приложение из private-чата по inline-кнопке или через кнопку меню бота.`, 'info');
  }
});
